...
. The first parameter is the group name, it identifies your scenario within the report.
    - You can use a namespace-like naming with dots to introduce a hierarchy (eg: analytics.funnels.tickets_sales)
...
Step methods are the heart of JsGiven. The test initialization, execution and assertions **must** be implemented in step methods.
...
In particular, no annotations are needed.
